...
pkgs.writeScriptBin "opener" ''
  ...
  function launch_gui() {
      if [ "$detatch" -eq 0 ]; then
          ...
          precmd="${pkgs.util-linux}/bin/setsid -f"
      fi
  ...
  function launch_term() {
      if [ "$detatch" -eq 1 ]; then
          ...
          ${pkgs.util-linux}/bin/setsid -f $TERMINAL -e $SHELL -c "$command_string; exec $SHELL" >/dev/null 2>&1
      else
  ...
  case $(${pkgs.file}/bin/file --mime-type "$1" -b) in
      ...
      text/*)
          ...
          launch_term $EDITOR "$1"
          ;;
      image/*)
          launch_gui ${pkgs.imv}/bin/imv "$1"
          ;;
      video/*) 
      ...
      image/vnd.djvu)
          launch_gui ${pkgs.zathura}/bin/zathura "$1"
          ;;
      audio/*|\
      video/x-ms-asf)
