# IPv4 manipulation functions
# from https://github.com/infinisil/system/blob/f41c1437aa146fcfd038694d92a077a02f01f142/deploy/lib/ip.nix
lib: rec {
  parseIp = str: map lib.toInt (builtins.match "([0-9]+)\\.([0-9]+)\\.([0-9]+)\\.([0-9]+)" str);
  prettyIp = lib.strings.concatMapStringsSep "." toString;
  ...
  parseSubnet = str:
    let
      ...
      warn = if baseIp == givenIp then lib.id else
      lib.warn
  ...
  isAddrFromSubnet = addr: subnet:
    (parseSubnet subnet).check (parseIp addr);
}
